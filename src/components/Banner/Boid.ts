...
function scale(number: number, fromLeft: number, fromRight: number, toLeft:number, toRight:number) {
...
class Boid {
  ...
  constructor(x: number, y: number, canvasRef: React.RefObject<HTMLCanvasElement>) {
  ...
  localFlock(boids: Boid[]) : Boid[] {
    return boids.filter((boid) => {
      ...
      const squareDist = (this.pos.x - boid.pos.x) ** 2 + (this.pos.y-boid.pos.y)**2
  ...
  repulsion(flock: Boid[], mousePos?: Vector2D) {
    ...
    if (mousePos !== undefined && (this.pos.x - mousePos.x)**2+(this.pos.y - mousePos.y)**2 < (4*FLOCK_RADIUS) ** 2) {
  ...
  draw (canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, mousePosRef: React.MutableRefObject<Vector2D | undefined>, color?: string) {
    ...
    if (mousePos !== undefined) {
      const squareDist = (this.pos.x-mousePos.x)**2+(this.pos.y-mousePos.y)**2;
      const alpha = scale(squareDist, 0, 16*FLOCK_RADIUS**2, 1, MIN_ALPHA);
      ctx.fillStyle = `rgba(${color ?? BOID_COLOR},${Math.max(MIN_ALPHA, alpha)})`;
    }
  ...
  update(boids: Boid[], mousePosRef: React.MutableRefObject<Vector2D | undefined>) {
